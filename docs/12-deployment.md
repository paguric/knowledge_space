...
Distribuire Knowledge Space come container Docker (backend REST) e come eseguibile Windows `.exe` (backend + frontend + webview).
...
| Base con CUDA | Embedding GPU | >2GB |
...
- **Dipende da:** Step 15 (REST), Step 16 (Frontend)
- **Usato da:** nessuno (fase finale)
