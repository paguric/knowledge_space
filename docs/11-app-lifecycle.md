...
> **Stato:** fatto | **Step:** 8-ter | **Fase:** 1A | **Aggiornato:** 25 luglio 2026

## Panoramica

Descrive come Knowledge Space viene eseguito: entry point, modalità di esecuzione (CLI standalone / backend process), lifecycle del processo, relazione tra i componenti runtime e protocollo CLI ↔ backend.
...
**CLI standalone (Fase 1–3):** ogni comando è un processo separato che costruisce un `AppContext` temporaneo, esegue l'operazione, salva lo stato su disco e termina. Nessun processo in background, watcher non attivi, sincronizzazione esplicita (`ks reindex`, `ks sync`).

**Backend process (Fase 4+):** `ks serve` avvia un processo long-running che unifica watcher, REST API, MCP SSE e (opzionalmente) frontend.
...
**Watcher watchdog:** un `WorkspaceWatcher` per ogni workspace attivo. Monitora le cartelle dei file sorgente (non `.knowledge-space/`). Su `modified`/`created`/`moved`/`deleted` → notifica il `KnowledgeBaseManager`.
...
| Dipende da | Usato da |
|------------|----------|
| Step 8-ter (AppContext), Step 12 (logging) | Step 13 (CLI), Step 14 (MCP), Step 15 (REST) |
