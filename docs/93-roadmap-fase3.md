...
- [ ] **Autocompletamento**: implementare shell completion (Typer nativa) per tutti i comandi; in particolare per `config set`/`unset` completare `<TAB>` sui percorsi dotted (`key`) e sui valori ammissibili (`value`) ricavati dal registry delle strategie. Prevedere alias brevi (es. `conf` → `config`).
